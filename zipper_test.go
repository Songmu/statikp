...
func TestZip(t *testing.T) {
	...
	out, err := statikp.Zip(
		"testdata/ziptree",
		statikp.FixMtime(time.Date(2000, time.January, 1, 0, 0, 0, 0, time.UTC)))
	if err != nil {
...
func TestFprintZipData(t *testing.T) {
	buf := &bytes.Buffer{}
	err := statikp.FprintZipData(buf, zipData4Print)
	if err != nil {
		t.Errorf("error should be nil but: %s", err)
	}
	out := buf.String()
...
func TestZip_CollectFile(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			dir:         "testdata/ziptree-skipdir",
			opts:        []statikp.Option{statikp.IncludeDotFiles(true)},
	...
	for _, tc := range tests {
		t.Run(tc.description, func(t *testing.T) {
			out, err := statikp.Zip(tc.dir, tc.opts...)
			if err != nil {
				t.Errorf("error should be nil, but: %s", err)
			}
